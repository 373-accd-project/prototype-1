...
class OESController < ApplicationController
  ...
  def download_csv
    @manager = JsonManager.new("https://api.bls.gov/publicAPI/v2/timeseries/data/")
    generated_id = "ENU" + session[:seasonal_adjustment_code] + session[:occupation_code] + session[:industry_code] + session[:area_code] + session[:area_type_code] + session[:data_type_code]
    puts generated_id
    parsed_json = JSON(@manager.apiCall(:generated_id, 2010, 2020))
